...
class Post extends Component {
  ...
  constructor(props) {
      super(props);
      _routeData = this.props.route;
      _post = _routeData.post;
  }
  ...
  renderListViewRow(row){
    if(row.count==_post.kids.length){
      return(
  ...
  renderListViewHeader(){
    ...
    <View style={styles.headerContainer}>
      ...
      <Text style={styles.headerPostDetailsLine}>
        Posted by {_post.by} | {_post.score} Points
      </Text>
      <View style={styles.separator}/>
      <Text style={styles.headerCommentTitle}>
        _post.descendants} Comments:
      </Text>
    </View>
    );
  }
  ...
  listViewOnRefresh(page, callback) {
    if(_post === null || _post.kids.length === 0){
      callback([], {allLoaded: true})
    }
    else if(page!=1){
      this.fetchCommentsUsingKids(_post.kids, this.state.lastIndex, 3, callback);
    }
    else{
      this.fetchCommentsUsingKids(_post.kids, 0, 3, callback);
    }
  }
